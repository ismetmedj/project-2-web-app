...
async function isAdmin() {
    ...
    if (
        TT.data.admin === user.data._id ||
        TT.data.editors.includes(user.data._id)
    ) {
        authRight = true
    }
}
...
async function printEvent(oneEvent) {
    ...
    document.getElementById('afficheEvent').style.backgroundColor =
        oneEvent.color
...
async function editForm(oneEvent) {
    ...
    document
        .querySelector('#editDiv>#newEvent>#title')
        .setAttribute('value', oneEvent.title)
    document
        .querySelector('#editDiv>#newEvent>#content')
